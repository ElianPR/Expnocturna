    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl max-w-2xl mx-auto">
        ...
        <div class="bg-blue-50/50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4">
            <div class="flex">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-blue-800 dark:text-blue-300">Información sobre el archivo</h3>
                    <div class="mt-2 text-sm text-blue-700 dark:text-blue-400">
                        <p><strong>Archivo .MP4:</strong> Ahora solo necesitas subir un único archivo de video en formato MP4 con fondo de color verde puro (chroma key).</p>
                        <p class="mt-1"><span class="text-xs opacity-75">Actualmente: {{ $cameraAnimation->mp4_file ? 'Subido' : 'No disponible' }}</span></p>
                        <p class="mt-2 text-xs">Deja el campo vacío si no deseas reemplazar la animación actual.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
            <form action="{{ route('camera-animations.update', $cameraAnimation) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium text-neutral-800 dark:text-neutral-200">Reemplazar Archivo .MP4 (Pantalla Verde)</label>
                    <input type="file" name="mp4_file" accept=".mp4" class="block w-full text-sm text-neutral-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-neutral-100 file:text-neutral-700 hover:file:bg-neutral-200 dark:file:bg-neutral-700 dark:file:text-neutral-200 dark:hover:file:bg-neutral-600 dark:text-neutral-400">
                    @error('mp4_file')
